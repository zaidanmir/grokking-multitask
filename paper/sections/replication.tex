...
the test loss stays at the random-guess level for $\sim 7{,}200$
...
operates on the multiplicative group $(\Z/p\Z)^*$ of order $p - 1$,
which is a different cyclic group; the published intuition is that
multiplication is harder to grok at the same step budget. We find
addition and subtraction grok within $\sim 200$ steps of each other
(Table \ref{tab:grok_steps_single}). At seed 42 multiplication groks
\emph{earlier} than the additive tasks --- the smaller operand domain
$\{1, \ldots, p - 1\}$ and the slightly different gradient signal from
having no zero-result row appear to favour faster discovery of the
multiplicative-group basis at this seed. Across other seeds (Section
\ref{sec:multitask}) multiplication is the slowest, restoring the
expected ordering.
...
similar step counts; multiplication groks fastest at this seed (4,200
vs 7,000--7,200 for the additive tasks), inverting the typically
reported ordering.}
...
\input{figures/tab1_grok_steps_single}
\end{table}
