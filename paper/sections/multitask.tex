...
Figure \ref{fig:fig3_multitask_two} shows per-task train and test
accuracy when a single network is trained jointly on addition and
subtraction. Both operations live on the additive group of $\Z/p\Z$, so
the natural prediction is that the model can solve both with a shared
additive Fourier basis. We find that the two tasks grok in close
proximity in step-count, supporting that prediction.
...
\input{figures/tab2_grok_steps_multi}
\end{table}
